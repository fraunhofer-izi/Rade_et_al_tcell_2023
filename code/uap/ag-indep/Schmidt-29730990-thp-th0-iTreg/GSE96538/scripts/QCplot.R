...
plotQualityProfile <- function(fl, n=1000000, aggregate=FALSE) {
...
manifest = read.table("~/projects/2018-MAVO/studies/kinetic/2018-Schmidt-29730990/GSE96538/scripts/rawdata-merge-lanes-manifest",
                      header = F, stringsAsFactors = F)
manifest = manifest$V1

manifest.R1 = manifest[grepl(".*R1.fastq.gz", manifest)]
manifest.R2 = manifest[grepl(".*R1.fastq.gz", manifest)]

png("qcplot_merged_langes_R1.png", width = 800, height = 800)
plotQualityProfile(sort(manifest.R1[1:33]), n=1000000)
dev.off()

png("qcplot_merged_langes_R2.png", width = 800, height = 800)
plotQualityProfile(sort(manifest.R2[1:33]), n=1000000)
dev.off()
