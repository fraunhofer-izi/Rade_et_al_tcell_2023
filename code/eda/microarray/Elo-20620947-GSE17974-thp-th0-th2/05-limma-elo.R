...
eda = readRDS(paste0(workdata, manifest$eda_prep_elo_thp_th2))
eset = eda$esets$rma
...
lfc = log2(1.5)
alpha = 0.05
out = paste0(workdata, manifest$limma_elo_thp_th2)
...
# design = model.matrix(~  HOURS,  data=pData(eset.core))
# colnames(design) = gsub("HOURS", "h_", colnames(design))
# colnames(design)
#
# contrast = makeContrasts(
