...
lfc = log2(1.5)
alpha = 0.05
out = paste0(work, manifest$limma_thp_itreg)
...
##
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
eda.prep$htseq.l[["tuomela"]] = NULL
eda.prep$phenoData.l[["tuomela"]] = NULL

for (i in names(eda.prep$phenoData.l)) {
  eda.prep$phenoData.l[[i]] = subset(eda.prep$phenoData.l[[i]], CONDITION == "thp" | CONDITION == "itreg")
...
dds = cbind(dds.l[[1]], dds.l[[2]], dds.l[[3]])
