...
lfc = log2(1.5)
alpha = 0.05
out = paste0(work, manifest$limma_thp_th17)
...
##
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
eda.prep$htseq.l[["ullah"]] = NULL; eda.prep$phenoData.l[["ullah"]] = NULL
eda.prep$htseq.l[["schmidt"]] = NULL; eda.prep$phenoData.l[["schmidt"]] = NULL
eda.prep$htseq.l[["schmidt.ind"]] = NULL; eda.prep$phenoData.l[["schmidt.ind"]] = NULL

for (i in names(eda.prep$phenoData.l)) {
  eda.prep$phenoData.l[[i]] = subset(eda.prep$phenoData.l[[i]], CONDITION == "thp" | CONDITION == "th17")
...
dds$samples$BIOLOGICAL_REPLICATE = paste0("R", dds$samples$BIOLOGICAL_REPLICATE)
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Make design matrix
design = model.matrix(~ 0 + BIOLOGICAL_REPLICATE + HOURS,  data = dds.fltrd$samples)
# design = model.matrix(~ 0 + HOURS,  data = dds.fltrd$samples)
...
# contrast = makeContrasts(
#   HOURS_05h_vs_0h = h_0.5h-h_0h,
#   HOURS_1h_vs_0h = h_1h-h_0h,
#   HOURS_2h_vs_0h = h_2h-h_0h,
#   # HOURS_4h_vs_0h = h_4h-h_0h,
#   HOURS_6h_vs_0h = h_6h-h_0h,
#   HOURS_12h_vs_0h = h_12h-h_0h,
#   HOURS_24h_vs_0h = h_24h-h_0h,
#   HOURS_48h_vs_0h = h_48h-h_0h,
#   HOURS_72h_vs_0h = h_72h-h_0h,
#   levels = colnames(design)
# )
